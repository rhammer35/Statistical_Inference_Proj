...
### 1. Simulations
To begin, we will need to run 1000 simulations of 40 exponential random variables. The assignment requires using a lambda value of 0.2. It should be noted that the exponential distribution is related to the geometric distribution, which is a measure of the number of binomial trials required to achieve a single success. The exponential distribution is the continuous version of the discrete geometric distribution.
For the simulation, I will use the rexp() function to generate exponential random variables. I will use a for loop to create a matrix storing each simulation of 40 random exponentials to a single row for 1000 rows. That code follows.
...
Next, I will take the mean and variance of each row of the matrix and store those values in vectors called "simmeans" and "simvars" respectively.
